    ...
    switch (index_args(argc, argv)) {
        case SHOW_HELP:
            printf ("\n"
                ...
                "    -ka (default %s)\n"
                "        Enable kill all job, soft include -k usage: lsf_taskmgr -ka\n"
                "    -f (default %s)\n"
                ...
                WINDOW_TITLE, (KILL_CONFIG & KILL_ENABLE) ? "TRUE" : "FALSE", (KILL_CONFIG & KILL_ALL_ENABLE) ? "TRUE" : "FALSE", 
                MONO_FONT, CPU_USAGE_THRESHOLD, AUTO_REFRESH_JOBS, AUTO_REFRESH_NODE, AUTO_REFRESH_RSRC);
